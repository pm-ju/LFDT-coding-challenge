...
use lockness_challenge::{decrypt, encrypt_to_bytes, Ciphertext, Error};
use rand::rngs::OsRng;

fn run<E: Curve>(label: &str) -> Result<(), Error> {
    let sk = SecretScalar::<E>::random(&mut OsRng);
    let pk = Point::generator() * &sk;

    let msg = b"Lockness Mentorship 2026";
    let ct_bytes = encrypt_to_bytes::<E>(&pk, msg, &mut OsRng)?;
    let ct = Ciphertext::<E>::from_bytes(&ct_bytes)?;
    let plaintext = decrypt::<E>(&sk, &ct)?;
